...
(package! shell-maker
  :recipe (:type git :host github :repo "xenodium/chatgpt-shell" :files ("shell-maker.el")))
...
(package! doom-nano-modeline
  :recipe (:type git :host github :repo "ronisbr/doom-nano-modeline"))
...
(when IS-MAC (setq frame-title-format nil))
(when IS-LINUX (setq frame-title-format '("%b — Emacs")))
...
(use-package! doom-nano-modeline
  ...
  (setq  doom-nano-modeline-top-padding 0.1
         doom-nano-modeline-bottom-padding 0.1)

  (defun doom-nano-modeline--space ()
  "Function to return a space for the modeline render function."
  `(("" . nil)))

  (defun mb/doom-nano-modeline-local-disable ()
    (interactive)
    "Disables the modeline locally in a buffer"
    (setq-local doom-nano-modeline-mode nil))

  (defun doom-nano-modeline--vterm-mode ()
    "Render the modeline in `vterm-mode'."
    (doom-nano-modeline--render
     `((,(abbreviate-file-name default-directory)
        . doom-nano-modeline-major-mode-face)) nil t)))
...
Miscellaneous.
#+begin_src elisp
(unbind-key "C-<tab>" evil-normal-state-map)
(unbind-key "C-<tab>" evil-visual-state-map)
(unbind-key "C-<tab>" evil-insert-state-map)
(map!
 :n "C-s" #'save-buffer
...
(defun mb/org-tree-slide-mode-hook ()
  (spell-fu-mode -1)
  (setq evil-motion-trainer-mode nil)
...
** kubernetes
#+begin_src elisp
(setq kubel-kubectl "/etc/profiles/per-user/mbaillie/bin/kubectl")
...
(after! lsp-mode
  ...
  ;; Annoying that this is in the global LSP workspace scope but it's the only
  ;; way I could make it work.
  (add-hook 'lsp-after-initialize-hook (lambda () (lsp--set-configuration
                                                   (list :formatting
                                                         (list :StringStyle "double"))))))
#+end_src
...
(defun mb/reset-company-complete-selection ()
  (map! (:map company-active-map
              "RET" #'company-complete-selection
              "[return]" #'company-complete-selection)))
(add-hook! 'company-mode-hook :append #'mb/reset-company-complete-selection)
#+end_src

** openai
#+begin_src elisp
(setf (alist-get (rx bos (or "*dall-e*" "*chatgpt*" "*ChatGPT*") eos)
...
(after! chatgpt-shell
  (setq
   chatgpt-shell-model-version "gpt-4-32k"
  ...
  (map!
   :after comint
   :map comint-mode-map
   :nv "RET" 'vterm-exit-visual-insert
   :vni "C-SPC" 'evil-normal-state
...
(use-package! gptel
  ...
  (defun gptel-quick (prompt)
    (interactive (list (read-string "ChatGPT: " nil gptel-quick--history)))
...
(after! vertico
  ...
  (custom-set-faces!
    '(vertico-posframe-border :inherit default :background "#bd93f9")))
#+end_src
...
#+begin_src elisp
(defun mb/show-welcome-buffer ()
  "Show *scratch* buffer."
  (with-current-buffer (get-buffer-create "*scratch*")
    (setq truncate-lines t)
    (let* ((buffer-read-only)
           (image-path (expand-file-name "unicorn.png" doom-private-dir))
...
#+begin_src elisp
(defun mb/insert-todo-here ()
  "Add a TODO item."
  (interactive)
  (insert "TODO(mbaillie): ")
  (evil-insert-state 1))
(map!
 :leader
 (:prefix-map ("i" . "insert")
  :desc "TODO item here" "t" #'mb/insert-todo-here))
#+end_src
